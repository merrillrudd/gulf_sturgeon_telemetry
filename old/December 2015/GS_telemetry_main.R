...
NOAAtags <- read.csv(file.path(data_csv_dir, "ALLV16_9.12_OFFICIAL MR.csv"), stringsAsFactors=FALSE)
NRDAtags <- read.csv(file.path(data_csv_dir, "Tags_vs_Forklength.csv"), stringsAsFactors=FALSE)
tag_update <- read.csv(file.path(data_csv_dir, "master_list_of_telemetered_fish_sept_2016.csv"), stringsAsFactors=FALSE)

## many more years in new tag update
noaa_yrs <- unique(NOAAtags$Year)[order(unique(NOAAtags$Year))]
nrda_yrs <- as.numeric(unique(sapply(1:nrow(NRDAtags), function(x) unlist(strsplit(NRDAtags$Date[x], "/"))[[3]])))[order(as.numeric(unique(sapply(1:nrow(NRDAtags), function(x) unlist(strsplit(NRDAtags$Date[x], "/"))[[3]]))))]
update_yrs <- as.numeric(unique(sapply(1:nrow(tag_update), function(x) unlist(strsplit(tag_update$Date[x], "/"))[[3]])))[order(as.numeric(unique(sapply(1:nrow(tag_update), function(x) unlist(strsplit(tag_update$Date[x], "/"))[[3]]))))]

noaa_trans <- NOAAtags$Transmitter
    length(noaa_trans)
    length(unique(noaa_trans))
nrda_trans <- NRDAtags$V_TagID
    length(nrda_trans)
    length(unique(nrda_trans))
update_trans <- tag_update$vTagID
    length(update_trans)
    length(unique(update_trans))

tag_update$Year <- as.numeric(sapply(1:nrow(tag_update), function(x) unlist(strsplit(tag_update$Date[x], "/"))[[3]]))
