...
par(mfrow=c(2,2),mai=c(0.5,0.5,0.5,0.1),omi=c(0.3,0.3,0,0))
boxplot(S.est.A21.s[,"A"],S.est.A21.l[,"A"],
		S.est.B21.s[,"B"],S.est.B21.l[,"B"],
		S.est.C21.s[,"C"],S.est.C21.l[,"C"],
		col=c(rep(c("gray25","gray75"),3)),main="Riverine Survival")
	abline(h=survR,col="red",lwd=3)
	axis(1,at=1:length(ax.labels),labels=ax.labels)
boxplot(S.est.A21.s[,"M"],S.est.A21.l[,"M"],
		S.est.B21.s[,"M"],S.est.B21.l[,"M"],
		S.est.C21.s[,"M"],S.est.C21.l[,"M"],
		col=c(rep(c("gray25","gray75"),3)),main="Marine Survival")
	abline(h=survM,col="blue",lwd=3)
	axis(1,at=1:length(ax.labels),labels=ax.labels)
boxplot(p.est.A21.s[,1],p.est.A21.l[,1],
		p.est.B21.s[,1],p.est.B21.l[,1],
		p.est.C21.s[,1],p.est.C21.l[,1],
		col=c(rep(c("gray25","gray75"),3)),main="Detection Probability")
	abline(h=pseason,col="forestgreen",lwd=3)
	axis(1,at=1:length(ax.labels),labels=ax.labels)
boxplot(rr.est.A21.s[,"A"],rr.est.A21.l[,"A"],
		rr.est.B21.s[,"B"],rr.est.B21.l[,"B"],
		rr.est.C21.s[,"C"],rr.est.C21.l[,"C"],
		col=c(rep(c("gray25","gray75"),3)),main="Fidelity Rate")
	abline(h=(1-emig),col="goldenrod",lwd=3)
	axis(1,at=1:length(ax.labels),labels=ax.labels)
mtext("Monitoring time, Simulated sample size of Gulf sturgeon tagged annually",side=1,outer=TRUE)
mtext("Estimates from Simulation",side=2,outer=TRUE)
