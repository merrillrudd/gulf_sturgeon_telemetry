...
###############################################
out_dir <- file.path(init_dir, "all_tags")
...
## data frame of  from each receiver
...
GSdets <- readRDS(file.path(inputs_rds, "filtered_GSdets.rds"))
    ...
    trans_nodet <- transmitters[which(transmitters$vTagID %in% trans_det == FALSE),]
    write.csv(trans_nodet, file.path(inputs_csv, "Transmitters_not_detected.csv") )
